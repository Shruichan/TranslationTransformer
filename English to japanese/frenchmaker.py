...
# Read dataset from file and clean sentences
with open(file_path, 'r', encoding='utf-8') as file:
    ...
    for line in lines:
        ...
        french_sentence = clean_sentence(parts[1])
        data["English"].append(english_sentence)
        data["French"].append(french_sentence)
...
train_dataset = TranslationDataset(train_df["English"].tolist(), train_df["French"].tolist(), tokenizer, tokenizer)
val_dataset = TranslationDataset(val_df["English"].tolist(), val_df["French"].tolist(), tokenizer, tokenizer)
...
model_save_path = "bert_translation_model_french_edition.pth"
